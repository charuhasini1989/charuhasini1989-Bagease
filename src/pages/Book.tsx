...
const Book = () => {
    ...
    useEffect(() => {
        ...
        const checkUserSession = async () => {
            try {
                // Add slight delay to ensure auth state is potentially settled if page loads fast
                // await new Promise(resolve => setTimeout(resolve, 50));
                const { data: { session }, error } = await supabase.auth.getSession();
                if (!isMounted) return;
                if (error) {
                    console.error("Auth Session Error:", error.message);
                    // Don't throw here, just set state
                    setIsAuthenticated(false);
                    setCurrentUser(null);
                } else if (!session) {
                    console.log("User not authenticated (getSession).");
                    setIsAuthenticated(false);
                    setCurrentUser(null);
                } else {
                    console.log("User authenticated (getSession):", session.user.id);
        ...
        const { data: authListener } = supabase.auth.onAuthStateChange((_event, session) => {
            if (isMounted) { // Removed !isLoadingAuth check here, listener should always update state if mounted
                 const currentlyAuth = !!session;
                 console.log(`Auth state changed via listener: User is now ${currentlyAuth ? 'authenticated' : 'not authenticated'}. Current state: ${isAuthenticated}`);
                 // Only update if state actually changes
                 if (currentlyAuth !== isAuthenticated) {
                    setIsAuthenticated(currentlyAuth);
                    setCurrentUser(session?.user ?? null);
                    if (currentlyAuth && session) {
                         console.log("Listener updating user data...");
                         setBookingData(prev => ({
                            ...prev,
                            email: prev.email || session.user?.email || '',
                            name: prev.name || session.user?.user_metadata?.full_name || ''
                         ...
                         console.log("Listener detected logout.");
    ...
    }, [isAuthenticated]); // Rerun effect if isAuthenticated changes (e.g., manual logout elsewhere)
    ...
    // --- Client-Side Assignment Function (WITH LOGGING) ---
    // *** ADDED setSubmitError as a dependency ***
    const assignRandomDeliveryPerson = useCallback(async (orderId: number, userId: string): Promise<boolean> => {
        console.log(`>>> AssignFunc: Attempting assignment for order ${orderId}...`); // LOG ADDED
        try {
            // 1. Get random personnel ID
            console.log(">>> AssignFunc: Calling RPC get_random_active_personnel..."); // LOG ADDED
            ...
            console.log(`>>> AssignFunc: RPC Success. Selected personnel ID: ${personnelId}`); // LOG ADDED

            // 2. Create assignment
            console.log(">>> AssignFunc: Inserting into delivery_assignments..."); // LOG ADDED
            ...
            console.log(`>>> AssignFunc: Assignment insert SUCCESS.`); // LOG ADDED

            // 3. Update order status
            console.log(">>> AssignFunc: Updating orders status..."); // LOG ADDED
            ...
            console.log(`>>> AssignFunc: Order status update SUCCESS.`); // LOG ADDED

            return true; // Success

        } catch (error: any) {
            console.error(`>>> ERROR inside assignRandomDeliveryPerson for order ${orderId}:`, error); // LOG ADDED
            // Use setSubmitError from the parent component's state
            setSubmitError(`Booking confirmed, but failed to assign delivery: ${error.message}. Please contact support if needed.`);
            return false; // Failure
        }
     // *** ADDED setSubmitError dependency ***
    }, [setSubmitError]);


    // --- Handle Submit (WITH LOGGING) ---
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        e.preventDefault();
        console.log(">>> handleSubmit triggered. isAuthenticated:", isAuthenticated, "currentUser:", currentUser?.id); // LOG ADDED
        if (!isAuthenticated || !currentUser) {
             console.log(">>> handleSubmit aborted: User not authenticated or currentUser is null."); // LOG ADDED
             setSubmitError("Authentication error. Please try logging out and back in.");
             // Consider forcing a re-check or logout action here if state seems inconsistent
             // await supabase.auth.signOut(); // Example: Force logout if state is broken
             setIsAuthenticated(false);
             setCurrentUser(null);
             return;
        }
        ...
        setErrors({}); // Clear validation errors at start of new submit attempt

        // Re-validate just before submitting
        const isValid = validateForm();
        console.log(">>> Form validation result:", isValid); // LOG ADDED
        ...
        if (isValid) {
            ...
            try {
                // Ensure currentUser is definitely available one last time
                 if (!currentUser || !currentUser.id) {
                    throw new Error("Critical Error: User ID became unavailable just before submit.");
                 }

                // 1. Prepare Booking Data
                console.log(">>> Step 1: Preparing booking payload..."); // LOG ADDED
                const bookingPayload = {
                    user_id: currentUser.id, // Use validated currentUser.id
                ...
                console.log(">>> Step 1: Verifying User ID for Booking Insert:", currentUser?.id); // LOG ADDED
                console.log(">>> Step 1: Submitting booking payload:", bookingPayload); // LOG ADDED
                ...
                // *** THIS IS WHERE YOUR ERROR IS REPORTEDLY HAPPENING ***
                if (bookingError) {
                    console.error(">>> RAW Supabase booking insert error:", bookingError); // LOG ADDED
                ...
                console.log('>>> Step 2: Booking insert SUCCESS! ID:', insertedBookingId); // Should see this if bookings works

                // 3. Prepare and Insert into 'orders' table
                console.log(">>> Step 3: Preparing order payload..."); // LOG ADDED
                const orderPayload = {
                    user_id: currentUser.id, // Use validated currentUser.id
                ...
                console.log(">>> Step 3: Verifying User ID for Order Insert:", currentUser?.id); // LOG ADDED
                console.log(">>> Step 3: Submitting order payload:", orderPayload); // LOG ADDED
                ...
                if (orderError) {
                     console.error(">>> RAW Supabase order insert error:", orderError); // LOG ADDED
                     throw new Error(`Order creation failed: ${orderError.message}`);
                }
                if (!insertedOrder?.id) throw new Error("Order created but failed to get ID.");
                insertedOrderId = insertedOrder.id;
                console.log(`>>> Step 3: Order created SUCCESS! ID: ${insertedOrderId}`);

                // 4. Assign Delivery Person
                console.log(">>> Step 4: Calling assignRandomDeliveryPerson..."); // LOG ADDED
                assignmentAttempted = true;
                assignmentSucceeded = await assignRandomDeliveryPerson(insertedOrderId, currentUser.id); // Pass validated currentUser.id
                console.log(">>> Step 4: assignRandomDeliveryPerson returned:", assignmentSucceeded); // LOG ADDED

                // 5. Handle overall success/partial success
                console.log(">>> Step 5: Handling final success state."); // LOG ADDED
                setIsSubmitSuccess(true); // Show success overlay
                // The error message for assignment failure is set within assignRandomDeliveryPerson's catch block
                if (!assignmentSucceeded) {
                    console.warn(">>> Step 5: Booking/Order succeeded, but assignment failed.");
                } else {
                    setSubmitError(null); // Ensure no residual error message on full success
                }

            } catch (err: any) {
                console.error('>>> ERROR during submission process:', err); // LOG ADDED - Captures errors thrown in the try block
                ...
                setErrors({}); // Clear validation errors as this is a submit-time error
                 ...
                 console.log(">>> Finally block executing. insertedOrderId:", insertedOrderId); // LOG ADDED
                 // Reset form ONLY if booking+order succeeded (even if assignment failed)
                if (insertedOrderId) {
                    console.log(">>> Resetting form after successful order creation."); // LOG ADDED
                    ...
                    setTimeout(() => {
                     ...
                     console.log(">>> Order creation failed or not reached, only setting isSubmitting false."); // LOG ADDED
                     // If booking or order insert failed, just stop submitting indicator
                     setIsSubmitting(false);
                }
            }
        } else {
            // Validation failed - errors state is already set by validateForm
            console.log(">>> Validation failed with errors:", errors); // LOG ADDED
     ...
     // *** Added assignRandomDeliveryPerson to dependencies ***
    }, [bookingData, validateForm, isAuthenticated, currentUser, errors, assignRandomDeliveryPerson, getMinDateTime, navigate]);
                        ...
                        <div className="p-3 bg-red-100 border-l-4 border-red-500 text-red-700 rounded-md text-sm mb-6 flex" role="alert">
                            ...
                            <div>
                                ...
                                <p className="font-bold">
                                    {currentErrorCount > 0 && Object.keys(errors).length > 0 ? 'Validation Errors Found' : 'Oops! Something went wrong'}
                                </p>
                                ...
                                {currentErrorCount > 0 && Object.keys(errors).length > 0 && (
