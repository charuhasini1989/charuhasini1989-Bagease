...
// --- Profile Form Data Type --- END

// --- Address Form Data Type --- START
interface AddressFormData {
    primary_address: string;
}
// --- Address Form Data Type --- END


// --- UPDATED: Booking Data Type (Reflecting Schema) --- START
interface Booking {
    // Core Identifiers & Timestamps
    id: number; // Assuming int8, adjust to string if UUID
    created_at: string; // timestamptz
    updated_at: string; // timestamptz
    user_id?: string; // uuid, foreign key

    // Contact Info (Assuming still present in table)
    name: string; // text
    phone: string; // text

    // Pickup Details
    pickup_address: string; // text
    pickup_location_type: string; // location_type (enum as string)
    pickup_date: string; // date
    pickup_time: string; // time

    // Drop Details
    drop_address: string; // text
    drop_location_type: string; // location_type (enum as string)

    // Train/Transport Details (Nullable)
    train_number?: string | null; // text
    train_name?: string | null; // text
    pnr_number?: string | null; // text
    coach_number?: string | null; // text
    seat_number?: string | null; // text

    // Item Details (Nullable)
    number_of_bags?: number | null; // int8
    weight_category?: string | null; // weight_category (enum as string)
    special_items_description?: string | null; // text
    insurance_requested?: boolean | null; // bool

    // Status & Payment (Nullable where applicable)
    booking_status: string; // booking_status_enum (enum as string) - Assuming non-null
    payment_status?: string | null; // payment_status_enum (enum as string)
    payment_mode?: string | null; // payment_mode_enum (enum as string)
    estimated_cost?: number | null; // numeric
    final_cost?: number | null; // numeric

    // Other Details (Nullable)
    delivery_preferences?: string | null; // delivery_preference (enum as string)
    rfid_tracking_id?: string | null; // text
    service_type?: string | null; // service_type_enum (enum as string)
    assigned_agent_id?: string | null; // uuid
}
// --- UPDATED: Booking Data Type --- END
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  // --- State for Profile --- END

  // --- State for Address --- START
  ...
  // --- State for Address --- END


    // --- Listener for Auth State Changes (Supabase) ---
    // (No changes needed in this listener itself)
    useEffect(() => {
      setLoading(true);
  ...
  const inputClasses = (hasError: boolean = false) =>
    ...
    const fetchUserBookings = async (userId: string) => {
        ...
        try {
            ...
            // Select columns based on the updated Booking interface and schema
            const { data, error } = await supabase
                ...
                `)
                .eq('user_id', userId)
                .order('created_at', { ascending: false }); // Keep ordering by most recent

            if (error) throw error; // Throw error to be caught below

            console.log("Fetched bookings data:", data);

            // Cast the data to the updated Booking interface
            const fetchedBookings = data as Booking[];
            setUserBookings(fetchedBookings);
            setLatestBooking(fetchedBookings.length > 0 ? fetchedBookings[0] : null); // Update latest booking
            if (fetchedBookings.length === 0) console.log("No bookings found for this user.");

        } catch (err: any) {
            console.error("Error fetching bookings:", err);
            const friendlyError = getFriendlyErrorMessage(err);
            setFetchBookingsError(friendlyError.text); // Use friendly message
            setUserBookings([]); // Clear data on error
            setLatestBooking(null);
        } finally {
            setIsLoadingBookings(false); // Set loading false in finally block
        }
    };
        ...
        try {
            ...
            if (profileData) {
                ...
                setAddressFormData({
                 ...
                 originalAddressRef.current = profileData.primary_address || '';
            } else {
            ...
            const friendlyError = getFriendlyErrorMessage(err);
            setFetchProfileError(friendlyError.text);
            setProfileFormData({ full_name: '', phone: '', date_of_birth: '' });
            setAddressFormData({ primary_address: '' });
             originalAddressRef.current = '';
        } finally {
    ...
    const handleUpdateProfile = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        setProfileFeedback(null);
        const currentProfileData = { ...profileFormData };
        ...
        try {
            ...
            const { error } = await supabase
                ...
                .update({
                    ...
                    phone: currentProfileData.phone || null,
                    date_of_birth: currentProfileData.date_of_birth || null,
                    updated_at: new Date().toISOString(),
                })
            ...
            const friendlyError = getFriendlyErrorMessage(err);
            setProfileFeedback(friendlyError);
        } finally {
            setIsUpdatingProfile(false);
        }
    };

    // --- Add/Update User Address Handler ---
    // (No changes needed)
    const handleAddOrUpdateAddress = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const { error } = await supabase
                .from('profiles')
                .update({
                    primary_address: newAddress || null,
                    updated_at: new Date().toISOString(),
                })
                .eq('id', currentUser.id);
            ...
            originalAddressRef.current = newAddress;

        } catch (err: any) {
            console.error("Error updating address:", err);
            const friendlyError = getFriendlyErrorMessage(err);
            setAddressFeedback(friendlyError);
        } finally {
            setIsUpdatingAddress(false);
        }
    };
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200 flex-shrink-0">
          ...
          <button
            ...
            disabled={loading || isUpdatingProfile || isUpdatingAddress}
          >
            <X size={24} />
          </button>
        </div>
          ...
          {!loading && currentUser ? (
            <div className="space-y-6">
              ...
              <nav className="space-y-2">
                 ...
                 <button className={`flex items-center w-full px-4 py-3 rounded-lg text-left text-gray-700 hover:bg-orange-50 hover:text-orange-600 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-1 transition-colors ${activeSection === 'address' ? 'bg-orange-100 text-orange-700 font-medium' : ''}`} onClick={() => setActiveSection('address')}> <Home size={18} className="mr-3 flex-shrink-0" /> Manage Address </button>
              </nav>

              {/* Dashboard Content Display Area */}
              <div className="mt-6 p-4 bg-gray-50 rounded-lg min-h-[250px]">

                    {/* Loading/Error States for Bookings */}
                    {/* (No changes needed) */}
                    {(activeSection === 'orders' || activeSection === 'tracking' || activeSection === 'delivery') && (
                        <>
                            {isLoadingBookings && ( <div className="flex justify-center items-center py-10 text-gray-600"> <Loader2 size={24} className="animate-spin text-orange-600 mr-2" /> Loading bookings... </div> )}
                            {fetchBookingsError && !isLoadingBookings && ( <div className="text-center py-6 text-red-600"> <AlertCircle className="mx-auto mb-2" size={30} /> <p className="text-sm">{fetchBookingsError}</p> <button onClick={() => currentUser && fetchUserBookings(currentUser.id)} disabled={!currentUser || isLoadingBookings} className="mt-3 px-3 py-1 text-sm bg-orange-100 text-orange-700 rounded hover:bg-orange-200 disabled:opacity-50"> Retry </button> </div> )}
                            {!isLoadingBookings && !fetchBookingsError && userBookings.length === 0 && activeSection === 'orders' && ( <div className="text-center py-10 text-gray-500"> <Package size={30} className="mx-auto mb-2" /> <p>You haven't placed any bookings yet.</p> </div> )}
                        </>
                    )}
                    ...
                    {(activeSection === 'profile' || activeSection === 'address') && (
                        <>
                            {isLoadingProfile && ( <div className="flex justify-center items-center py-10 text-gray-600"> <Loader2 size={24} className="animate-spin text-orange-600 mr-2" /> Loading details... </div> )}
                            {fetchProfileError && !isLoadingProfile && ( <div className="text-center py-6 text-red-600"> <AlertCircle className="mx-auto mb-2" size={30} /> <p className="text-sm">{fetchProfileError}</p> <button onClick={() => currentUser && fetchUserProfile(currentUser.id)} disabled={!currentUser || isLoadingProfile} className="mt-3 px-3 py-1 text-sm bg-orange-100 text-orange-700 rounded hover:bg-orange-200 disabled:opacity-50"> Retry </button> </div> )}
                         </>
                    )}
                  ...
                  {/* --- My Orders Section --- */}
                  {/* UPDATED: Added pickup_date and estimated_cost display */}
                           ...
                           <ul className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
                               {userBookings.map((booking) => (
                                   <li key={booking.id} className="p-3 border rounded-md bg-white shadow-sm text-sm transition-shadow hover:shadow-md">
                                       ...
                                       {/* ADDED: Pickup Date */}
                                       <p className="text-xs text-gray-500 mb-1"> Pickup Date: {booking.pickup_date ? new Date(booking.pickup_date).toLocaleDateString() : 'N/A'} </p>
                                       ...
                                       {booking.estimated_cost !== null && booking.estimated_cost !== undefined && (
                                        <p className="text-xs text-gray-600 mt-1"> <span className="font-medium">Est. Cost:</span> ₹{booking.estimated_cost.toFixed(2)} </p>
                                       )}
                                   </li>
                               ))}
                           </ul>
                  ...
                  {/* UPDATED: Ensure latestBooking fields used still exist */}
                  {activeSection === 'tracking' && !isLoadingBookings && !fetchBookingsError && latestBooking && (
                      <div>
                            ...
                            <div className="space-y-2 text-sm">
                                ...
                                {/* Pickup Date added for context */}
                                <p><span className="font-medium">Pickup Date:</span> {latestBooking.pickup_date ? new Date(latestBooking.pickup_date).toLocaleDateString() : 'N/A'}</p>
                                <p><span className="font-medium">Pickup Address:</span> {latestBooking.pickup_address}</p>
                  ...
                  {activeSection === 'delivery' && !isLoadingBookings && !fetchBookingsError && latestBooking && (
                        <div>
                            ...
                            <div className="space-y-2 text-sm">
                                ...
                                {/* Use name/phone from booking record */}
                                <p><span className="font-medium">Recipient Name:</span> {latestBooking.name || 'N/A'}</p>
                                <p><span className="font-medium">Recipient Phone:</span> {latestBooking.phone || 'N/A'}</p>
                                ...
                                {latestBooking.coach_number && <p><span className="font-medium">Coach:</span> {latestBooking.coach_number}</p>}
                                {latestBooking.seat_number && <p><span className="font-medium">Seat:</span> {latestBooking.seat_number}</p>}
                            </div>
                        </div>
                   )}
                    {activeSection === 'delivery' && !isLoadingBookings && !fetchBookingsError && !latestBooking && (
                   ...
                   {activeSection === 'profile' && !isLoadingProfile && !fetchProfileError && (
                        <div>
                            ...
                            {profileFeedback && ( <div className={`mb-4 p-3 border rounded-md text-sm flex items-start ${ profileFeedback.type === 'error' ? 'bg-red-50 border-red-300 text-red-800' : profileFeedback.type === 'success' ? 'bg-green-50 border-green-300 text-green-800' : 'bg-blue-50 border-blue-300 text-blue-800' }`}> <AlertCircle size={18} className="mr-2 flex-shrink-0 mt-0.5" /> <span>{profileFeedback.text}</span> </div> )}
                            <form onSubmit={handleUpdateProfile} className="space-y-4">
                                <div> <label htmlFor="profile_full_name" className="block text-sm font-medium text-gray-700 mb-1">Full Name</label> <input id="profile_full_name" name="full_name" type="text" value={profileFormData.full_name} onChange={handleProfileInputChange} placeholder="Your full name" className={inputClasses(profileFeedback?.type === 'error' && profileFeedback.text.includes('name'))} required disabled={isUpdatingProfile} /> </div>
                                <div> <label htmlFor="profile_phone" className="block text-sm font-medium text-gray-700 mb-1">Phone Number</label> <input id="profile_phone" name="phone" type="tel" value={profileFormData.phone} onChange={handleProfileInputChange} placeholder="e.g., 9876543210" className={inputClasses(profileFeedback?.type === 'error' && profileFeedback.text.includes('phone'))} disabled={isUpdatingProfile} /> </div>
                                <div> <label htmlFor="profile_date_of_birth" className="block text-sm font-medium text-gray-700 mb-1">Date of Birth</label> <input id="profile_date_of_birth" name="date_of_birth" type="date" value={profileFormData.date_of_birth} onChange={handleProfileInputChange} max={new Date().toISOString().split("T")[0]} className={inputClasses()} disabled={isUpdatingProfile} /> </div>
                                <div> <label htmlFor="profile_email" className="block text-sm font-medium text-gray-700 mb-1">Email</label> <input id="profile_email" type="email" value={currentUser.email || ''} className={inputClasses() + ' bg-gray-100'} disabled readOnly title="Email cannot be changed here" /> <p className="text-xs text-gray-500 mt-1">Email cannot be changed.</p> </div>
                                <button type="submit" disabled={isUpdatingProfile} className="w-full bg-orange-600 text-white py-2.5 px-4 rounded-lg font-semibold hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 transition-colors disabled:opacity-70 flex items-center justify-center"> {isUpdatingProfile ? <Loader2 size={20} className="mr-2 animate-spin" /> : <UserCog size={18} className="mr-2" />} {isUpdatingProfile ? 'Saving...' : 'Save Changes'} </button>
                            </form>
                        </div>
                   )}
                    ...
                    {activeSection === 'address' && !isLoadingProfile && !fetchProfileError && (
                        <div>
                            <h3 className="text-lg font-semibold mb-3 text-gray-800 border-b border-gray-200 pb-1">Primary Address</h3>
                             {addressFeedback && ( <div className={`mb-4 p-3 border rounded-md text-sm flex items-start ${ addressFeedback.type === 'error' ? 'bg-red-50 border-red-300 text-red-800' : addressFeedback.type === 'success' ? 'bg-green-50 border-green-300 text-green-800' : 'bg-blue-50 border-blue-300 text-blue-800' }`}> <AlertCircle size={18} className="mr-2 flex-shrink-0 mt-0.5" /> <span>{addressFeedback.text}</span> </div> )}
                            <form onSubmit={handleAddOrUpdateAddress} className="space-y-4">
                                <div> <label htmlFor="primary_address" className="block text-sm font-medium text-gray-700 mb-1"> {originalAddressRef.current ? 'Edit Primary Address' : 'Add Primary Address'} </label> <textarea id="primary_address" name="primary_address" rows={4} value={addressFormData.primary_address} onChange={handleAddressInputChange} placeholder="Enter your full primary address (house no, street, city, state, pincode)" className={inputClasses(addressFeedback?.type === 'error') + ' resize-none'} disabled={isUpdatingAddress} /> <p className="text-xs text-gray-500 mt-1">This address may be used for future bookings.</p> </div>
                                <button type="submit" disabled={isUpdatingAddress} className="w-full bg-orange-600 text-white py-2.5 px-4 rounded-lg font-semibold hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 transition-colors disabled:opacity-70 flex items-center justify-center"> {isUpdatingAddress ? <Loader2 size={20} className="mr-2 animate-spin" /> : <Home size={18} className="mr-2" />} {isUpdatingAddress ? 'Saving...' : 'Save Address'} </button>
                            </form>
                        </div>
                    )}

                 {/* Message when no section selected */}
                 {/* (No changes needed) */}
